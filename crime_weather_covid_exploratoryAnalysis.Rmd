...
ggplot(counts_day_parts) + aes(x = date) + 
  ...
  theme_bw()
```
The very granular bar chart reveals the downward trend of crime counts over the years, but also a strong seasonal component within eath year. We also reveal daily extremes, or "spikes," were the counts are a lot higher than counts on the day before or after. We can also identify the steep drop of crime rates with the onset of COVID-19 (Mar 2020).
...
per_month %>%
  ...
  annotate("text", x = as.Date("2015-04-01"), y = 9000, label = "Year 2020", size = 3)
...
# group across offense types by day
daily_counts <- counts_day %>%
  dplyr::mutate(date = collapse_index(date, "daily")) %>%
  dplyr::group_by(date) %>%
...
### tstibble comes with numerous helpful functions
...
```{r monthly counts using }
# alternative approach (not using tsibble)
#monthly_counts <- counts_tbl %>%
#  dplyr::mutate(date = collapse_index(date, "monthly")) %>%
#  dplyr::group_by(date) %>%
#  dplyr::summarise(ttl_counts = sum(count))
```
